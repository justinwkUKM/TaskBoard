...
export function Modal({
  ...
  const wideClass = wide === 'extra' ? 'dialog-extra-wide' : wide ? 'dialog-wide' : '';
  return (
    <Dialog.Root open onOpenChange={open => { if (!open) onClose(); }}>
      <Dialog.Portal>
        <Dialog.Overlay className="dialog-overlay" />
        <Dialog.Content className={`dialog ${wideClass}`} onInteractOutside={event => event.preventDefault()}>
          <div className="dialog-heading">
            <div>
              <Dialog.Title>{title}</Dialog.Title>
              <Dialog.Description>{description || 'Make a little progress.'}</Dialog.Description>
            </div>
